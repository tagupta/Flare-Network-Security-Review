...
interface ICoreVaultClient {
    ...
    event TransferToCoreVaultStarted(
        address indexed agentVault, uint256 indexed transferRedemptionRequestId, uint256 valueUBA
    );
    ...
    event TransferToCoreVaultDefaulted(
        address indexed agentVault, uint256 indexed transferRedemptionRequestId, uint256 remintedUBA
    );
    ...
    event ReturnFromCoreVaultRequested(
        address indexed agentVault, uint256 indexed requestId, bytes32 paymentReference, uint256 valueUBA
    );
    ...
    event ReturnFromCoreVaultConfirmed(
        address indexed agentVault, uint256 indexed requestId, uint256 receivedUnderlyingUBA, uint256 remintedUBA
    );
    ...
    event CoreVaultRedemptionRequested(
        address indexed redeemer, string paymentAddress, bytes32 paymentReference, uint256 valueUBA, uint256 feeUBA
    );
     ...
     * @param _agentVault the agent vault address
     */
    function confirmReturnFromCoreVault(IPayment.Proof calldata _payment, address _agentVault) external;
     ...
     */
    function redeemFromCoreVault(uint256 _lots, string memory _redeemerUnderlyingAddress) external;

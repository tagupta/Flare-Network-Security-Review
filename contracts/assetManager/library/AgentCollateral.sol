...
library AgentCollateral {
    ...
    function combinedData(Agent.State storage _agent) internal view returns (Collateral.CombinedData memory) {
    ...
    function singleCollateralData(Agent.State storage _agent, Collateral.Kind _kind)
        internal
        view
    ...
    function agentVaultCollateralData(Agent.State storage _agent) internal view returns (Collateral.Data memory) {
    ...
    function poolCollateralData(Agent.State storage _agent) internal view returns (Collateral.Data memory) {
    ...
    function freeCollateralLots(Collateral.CombinedData memory _data, Agent.State storage _agent)
        internal
        view
    ...
    function freeCollateralLotsOptionalFee(
        ...
        bool _chargePoolFee
    ) internal view returns (uint256 _lots) {
    ...
    function freeSingleCollateralLots(Collateral.Data memory _data, Agent.State storage _agent, bool _chargePoolFee)
        internal
        view
        returns (uint256)
    {
    ...
    function freeCollateralWei(Collateral.Data memory _data, Agent.State storage _agent)
        internal
        view
        returns (uint256)
    {
        ...
        returns (uint256)
    {
        ...
        uint256 mintingCollateral =
            Conversion.convertAmgToTokenWei(backedAMG, _data.amgToTokenWeiPrice).mulBips(mintingMinCollateralRatioBIPS);
        uint64 redeemingAMG = _data.kind == Collateral.Kind.POOL ? _agent.poolRedeemingAMG : _agent.redeemingAMG;
        uint256 redeemingCollateral = Conversion.convertAmgToTokenWei(redeemingAMG, _data.amgToTokenWeiPrice).mulBips(
            systemMinCollateralRatioBIPS
        );
    ...
    function mintingLotCollateralWei(Collateral.Data memory _data, Agent.State storage _agent, bool _chargePoolFee)
        internal
        view
        returns (uint256)
    {
    ...
    function collateralRatioBIPS(Collateral.Data memory _data, Agent.State storage _agent)
        internal
        view
        returns (uint256)
    {
    ...
    function totalBackedAMG(Agent.State storage _agent, Collateral.Kind _collateralKind)
        internal
        view
        returns (uint256)
    {
